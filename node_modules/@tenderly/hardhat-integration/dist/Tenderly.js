...
class Tenderly {
    constructor(hre) {
        this.tenderlyService = new api_client_1.TenderlyService(constants_2.PLUGIN_NAME);
        logger_1.logger.debug("Creating Tenderly plugin.");
        this.env = hre;
        this.tenderlyNetwork = new TenderlyNetwork_1.TenderlyNetwork(hre);
        this.verificationService = new verification_1.VerificationService(this.tenderlyService, this.tenderlyNetwork);
    ...
    async verify(...contracts) {
        ...
        if (await this._isZkSyncNetwork(this.env)) {
            for (let contract of contracts) {
                contract = contract;
                this.verificationService.verifyContractABI(this.env, contract.address, contract.name);
            }
            return;
        }
        // If there are proxy contracts, we can run the task without further processing.
        const proxyContracts = contracts.filter((contract) => contract.name === index_1.ProxyPlaceholderName);
        ...
        const flatContracts = contracts.reduce((accumulator, value) => {
            if (value.name !== index_1.ProxyPlaceholderName) {
                accumulator = accumulator.concat(value);
            }
            return accumulator;
        }, []);
        if (flatContracts === undefined ||
            flatContracts === null ||
            flatContracts.length === 0) {
            return;
        }
        const verificationType = await (0, utils_1.getVerificationType)(this.env, this.tenderlyNetwork);
        const platformID = verificationType === api_client_1.VERIFICATION_TYPES.FORK
            ? this.tenderlyNetwork.forkID
            : this.tenderlyNetwork.devnetID;
        const requestData = await (0, util_1.makeVerifyContractsRequest)(this.env, flatContracts, platformID);
        if (requestData === null) {
        ...
        if (await (0, utils_1.isVerificationOnVnet)(verificationType)) {
            ...
            await (0, errors_2.throwIfUsernameOrProjectNotSet)(this.env);
            return this.tenderlyNetwork.verify(requestData);
        }
        if (verificationType === api_client_1.VERIFICATION_TYPES.PRIVATE) {
    ...
    async verifyMultiCompilerAPI(request) {
        ...
        switch (verificationType) {
            case api_client_1.VERIFICATION_TYPES.FORK:
                ...
                break;
            case api_client_1.VERIFICATION_TYPES.DEVNET:
                logger_1.logger.error(`Error in ${constants_2.PLUGIN_NAME}: Network parameter is set to '${this.getNetworkName()}' and verifyMultiCompilerAPI() is not available for devnet deployments`);
                break;
            case api_client_1.VERIFICATION_TYPES.PRIVATE:
                ...
                break;
            case api_client_1.VERIFICATION_TYPES.PUBLIC:
                logger_1.logger.info("Publicly verifying contracts.");
                await this.tenderlyService.verifyContractsMultiCompiler(request);
                break;
        }
    }
    async _isZkSyncNetwork(hre) {
        let chainId;
        try {
            chainId = await (0, util_1.getChainId)(hre);
        }
        catch (e) {
            if (e instanceof errors_2.UndefinedChainIdError) { }
            else
                throw e;
        }
    ...
    async verifyDevnetMultiCompilerAPI(request, tenderlyProject, username, devnetID) {
    ...
    async verifyAPI(request) {
        ...
        if ((0, util_1.isTenderlyNetworkConfig)(this.env.network.config)) {
            if (verificationType === api_client_1.VERIFICATION_TYPES.DEVNET) {
                logger_1.logger.error(`Error in ${constants_2.PLUGIN_NAME}: Network parameter is set to '${this.getNetworkName()}' and verifyAPI() is not available for devnet deployments.`);
                return;
            }
    ...
    async pushAPI(request, tenderlyProject, username) {
        ...
        if ((0, util_1.isTenderlyNetworkConfig)(this.env.network.config)) {
            if (verificationType === api_client_1.VERIFICATION_TYPES.DEVNET) {
                logger_1.logger.error(`Error in ${constants_2.PLUGIN_NAME}: Network parameter is set to '${this.getNetworkName()}' and pushAPI() is not available for devnet deployments.`);
                return;
            }
    ...
    _existsProxyVerification(contracts) {
        return contracts.some((contract) => contract.name === index_1.ProxyPlaceholderName);
    }
}
