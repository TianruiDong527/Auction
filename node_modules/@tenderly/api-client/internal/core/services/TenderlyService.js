...
class TenderlyService {
    ...
    async verifyContracts(request) {
        ...
        try {
            ...
            if (responseData.contracts === undefined ||
    ...
    async verifyContractABI(username, project, verificationType, request) {
        logger_1.logger.debug(`Verifying contract with ABI on ${verificationType}`);
        if (!TenderlyApiService_1.TenderlyApiService.isAuthenticated()) {
            logger_1.logger.error(`Error in ${this.pluginName}: ${errors_1.ACCESS_TOKEN_NOT_PROVIDED_ERR_MSG}`);
            throw new errors_1.UnauthorizedError();
        }
        if (verificationType === types_1.VERIFICATION_TYPES.PUBLIC || verificationType === types_1.VERIFICATION_TYPES.PRIVATE) {
    ...
    async verifyContractABIOnProject(username, project, verificationType, request) {
        const tenderlyApi = TenderlyApiService_1.TenderlyApiService.configureInstance();
        const isPublicVerification = verificationType === types_1.VERIFICATION_TYPES.PUBLIC;
        const res = await tenderlyApi.post(`/api/v1/account/${username}/project/${project}/contract/${request.networkId}/${request.address}/abi`, {
            contract_name: request.contractName,
            abi: request.abi,
            is_public_verification: isPublicVerification,
        });
        if (res.data === undefined || res.data === null) {
            throw new errors_1.InvalidResponseError("verifyContractABIOnProject", res.data);
        }
        const response = res.data;
        if (response.error) {
            throw new errors_1.BytecodeMissingMethodSignaturesError(response.error);
        }
        const contractLink = `${constants_1.TENDERLY_DASHBOARD_BASE_URL}/${username}/${project}/contract/${request.networkId}/${request.address}`;
        const logMsg = `Contract ${request.address} verified with ABI. You can view the contract at ${contractLink}`;
        console.log(logMsg);
        return response;
    }
    async verifyContractABIOnVnet(username, project, request) {
        const tenderlyApi = TenderlyApiService_1.TenderlyApiService.configureInstance();
        const res = await tenderlyApi.post(`/api/v1/account/${username}/project/${project}/testnet/${request.networkId}/contract/${request.address}/abi`, {
            contract_name: request.contractName,
            abi: request.abi,
        });
        if (res.data === undefined || res.data === null) {
            throw new errors_1.InvalidResponseError("verifyContractABIOnVnet", res.data);
        }
        const response = res.data;
        if (response.error) {
            throw new errors_1.BytecodeMissingMethodSignaturesError(response.error);
    ...
    async verifyContractsMultiCompiler(request) {
        ...
        try {
            ...
            if (response.compilation_errors !== undefined &&
            ...
            if (response.results.verified_contracts !== undefined &&
        ...
        try {
            ...
            if (response.results.verified_contracts !== undefined &&
                ...
                for (const verifiedContract of response.results.verified_contracts) {
                    ...
                    if (response.display_link != undefined &&
