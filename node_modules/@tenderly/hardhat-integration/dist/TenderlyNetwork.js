...
const log_compliance_1 = require("@tenderly/api-client/utils/log-compliance");
...
class TenderlyNetwork {
    constructor(hre) {
        ...
        if ((0, util_1.isTenderlyNetworkConfig)(hre.network.config) &&
            ...
            if (hre.network.config.url.includes("devnet")) {
                ...
                logger_1.logger.info("Devnet ID is:", this.devnetID);
            }
            else {
    ...
    async verify(requestData) {
        logger_1.logger.info("Invoked verification.");
        if (!this._checkNetwork()) {
            return;
        }
        if (this.devnetID !== undefined) {
            await this.tenderlyService.verifyDevnetContractsMultiCompiler(requestData, this.env.config.tenderly.project, this.env.config.tenderly.username, this.devnetID, this.env.config.tenderly.accessKey);
            return;
        }
    ...
    async verifyDevnetMultiCompilerAPI(request, tenderlyProject, username, devnetID) {
        logger_1.logger.info("Invoked devnet verification via API.");
        await this.tenderlyService.verifyDevnetContractsMultiCompiler(request, tenderlyProject, username, devnetID);
    }
    ...
    async initializeFork() {
        ...
        logger_1.logger.trace("ProjectID obtained from tenderly configuration:", {
            projectID,
        });
        try {
