...
const makeVerifyContractsRequest = async (hre, flatContracts, platformID) => {
    ...
    for (const flatContract of flatContracts) {
        ...
        let chainId;
        if ((0, exports.isTenderlyNetworkConfig)(hre.config.networks[networkName]) &&
...
async function getChainId(hre) {
    ...
    if (!chainId) {
        throw new errors_2.UndefinedChainIdError(networkName);
    }
...
async function insertLibraries(hre, originalCompiler, libraries) {
    ...
    if (copiedCompiler.settings.libraries !== undefined &&
...
const getCompilerDataFromContracts = (contracts, flatContracts, hhConfig) => {
    ...
    for (contract of contracts) {
        for (mainContract of flatContracts) {
            ...
            if (config !== null &&
                config !== undefined &&
                !(0, exports.compareConfigs)(contractConfig, config)) {
